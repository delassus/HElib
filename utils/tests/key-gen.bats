...
@test "creates the correct files given a prefix" {
  run "${key_gen}" "${prefix_bgv}.params" -o abcd
  assert [ "$status" -eq 0 ]
  assert [ -f "abcdEnc.pk" ]
  assert [ -f "abcdEval.pk" ]
  assert [ -f "abcd.sk" ]
}

@test "create BGV toy params with info file" {
  run "${key_gen}" "${prefix_bgv}.params" --info-file -o bcde
  assert [ "$status" -eq 0 ]
  assert [ -f "bcde.info" ]
}

@test "key switching matrices are created by default and indicated to cmd line" {
  run "${key_gen}" "${prefix_bgv}.params" -o a
  assert [ "$status" -eq 0 ]
...
@test "key switching matrices are created by default and indicated in the info-file" {
  run "${key_gen}" "${prefix_bgv}.params" --info-file -o defg
  assert [ "$status" -eq 0 ]
  assert [ -f "defg.info" ]
...
@test "no-skm flag disables generation of key switching matrices" {
  run "${key_gen}" "${prefix_bgv}.params" --info-file --no-skm -o cdef
  assert [ "$status" -eq 0 ]
  assert [ -f "cdef.info" ]
...
@test "no-skm and frob-skm flags are not a valid combination" {
  run "${key_gen}" "${prefix_bgv}.params" --no-skm --frob-skm -o cdef
  assert [ "$status" -ne 0 ]
...
@test "default scheme fails if p is less than 2" {
  run "${key_gen}" "${prefix_ckks}.params"
  assert [ "$status" -ne 0 ]
...
@test "BGV: fails if p is less than 2" {
  run "${key_gen}" "${prefix_ckks}.params" --scheme BGV
  assert [ "$status" -ne 0 ]
  assert [ "$output" == "BGV invalid plaintext modulus. In BGV it must be a prime number greater than 1." ]
}

@test "CKKS: fails if p does not equal -1" {
  run "${key_gen}" "${prefix_bgv}.params" --scheme CKKS
  assert [ "$status" -ne 0 ]
...
@test "BGV: fails on non-prime p" {
  ...
  run "${key_gen}" "${prefix_bgv}_non_prime_p.params" --scheme BGV
  assert [ "$status" -ne 0 ]
  assert [ "${lines[1]}" == "Modulus pp is not prime (nor -1)" ]
}

@test "scheme flag works correctly for BGV and CKKS" {
  run "${key_gen}" "${prefix_bgv}.params" --scheme BGV -o bgv
  assert [ "$status" -eq 0 ]
...
@test "CKKS: fails on odd m" {
  ...
  run "${key_gen}" "${prefix_ckks}_odd_m.params" --scheme CKKS
  assert [ "$status" -ne 0 ]
...
@test "CKKS: fails on m non-power of 2" {
  ...
  run "${key_gen}" "${prefix_ckks}_nonp2_m.params" --scheme CKKS
  assert [ "$status" -ne 0 ]
  assert [ "${lines[1]}" == "CKKS scheme only supports m as a power of two." ]
}

@test "fails on invalid scheme option" {
  run "${key_gen}" "${prefix_bgv}.params" --scheme BFV -o bfv
  assert [ "$status" -ne 0 ]
  assert [ "$output" == "Unrecognized scheme 'BFV'." ]
}

@test "bootstrap = NONE option works as normal" {
  run "${key_gen}" "${prefix_bgv}.params" --bootstrap NONE -o test
  assert [ "$status" -eq 0 ]
  assert [ -f "testEnc.pk" ]
  assert [ -f "testEval.pk" ]
  assert [ -f "test.sk" ]
}

@test "bootstrap = THIN option works with bootstrap params" {
  create-bootstrap-toy-params
  run "${key_gen}" "${prefix_bootstrap}.params" --bootstrap THIN -o thin
  assert [ "$status" -eq 0 ]
  assert [ -f "thinEnc.pk" ]
  assert [ -f "thinEval.pk" ]
  assert [ -f "thin.sk" ]
}

@test "bootstrap = THICK option works with bootstrap params" {
  create-bootstrap-toy-params
  run "${key_gen}" "${prefix_bootstrap}.params" --bootstrap THICK -o thick
  assert [ "$status" -eq 0 ]
  assert [ -f "thickEnc.pk" ]
  assert [ -f "thickEval.pk" ]
  assert [ -f "thick.sk" ]
}

@test "fails when passing invalid bootstrap option" {
  create-bootstrap-toy-params
  run "${key_gen}" "${prefix_bootstrap}.params" --bootstrap INVALID
  assert [ "$status" -ne 0 ]
...
@test "bootstrap fails when --no-skm is specified" {
  create-bootstrap-toy-params
  run "${key_gen}" "${prefix_bootstrap}.params" --bootstrap THIN --no-skm
  assert [ "$status" -ne 0 ]
...
@test "bootstrap fails when missing parameters (mvec, gens, ords)" {
  ...
  run "${key_gen}" "${prefix_bootstrap}_mvec.params" --bootstrap THICK
  assert [ "$status" -ne 0 ]
...
@test "bootstrap creates SKM, Frobenius matrices and recrypt data and indicates it to cmd line" {
  create-bootstrap-toy-params
  run "${key_gen}" "${prefix_bootstrap}.params" --bootstrap THICK -o defg
  assert [ "$status" -eq 0 ]
  assert [ "${lines[0]}" == "Key switching matrices created." ]
  assert [ "${lines[1]}" == "Frobenius matrices created." ]
...
@test "bootstrap creates SKM, Frobenius matrices and recrypt data and indicates it to the info-file" {
  create-bootstrap-toy-params
  run "${key_gen}" "${prefix_bootstrap}.params" --info-file --bootstrap THICK -o defg
  assert [ "$status" -eq 0 ]
  assert [ -f "defg.info" ]
