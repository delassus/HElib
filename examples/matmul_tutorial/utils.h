...
void printMatrix(const helib::Matrix<double>& M, const size_t& rmax, const size_t& cmax, std::ostream& out = std::cout)
{
...
void printMatrix(const std::vector<std::vector<T>>& M, const size_t& rmax, const size_t& cmax, std::ostream& out = std::cout)
{
...
template <typename T>
void printVector(const std::vector<T> &v, const size_t &cmax, std::ostream &out = std::cout)
{
 ...
 */
template <typename T>
void printVectorAsMatrix(const std::vector<T> &v, const size_t &dim0, const size_t &dim1, std::ostream &out = std::cout)
{
...
/**
 * @brief sample from a continuous Gaussian with mean mu, standard deviation sigma.
 * Adapts the Box-Muller method as implemented in sample.cpp of HElib
 * @param mu
 * @param sigma
 * @return double
 */
 ...
 * @return Matrix<double> dim0 x dim1 matrix with each entry sampled from [min,max]
 */
helib::Matrix<double> uniformMatrix(const long &dim0, const long &dim1, const double &min, const double &max)
{
...
std::vector<double> uniformVector(const long &dim, const double &min, const double &max)
{
    std::vector<double> v;
    v.reserve(dim);
    for (int i = 0; i < dim; i++)
 ...
 * @return vector<int> a vector with elements uniform in [min, min + 1,..., max - 1, max]
 */
...
{
    helib::assertEq<LogicError>(v1.size(), v2.size(), "can only dot two vectors of the same size");
    double dp = 0;
...
template <typename T>
std::vector<std::vector<T>> matrixProduct(const std::vector<std::vector<T>> &A, const std::vector<std::vector<T>> &B)
{
    helib::assertEq<LogicError>(B.size(), A[0].size(), "inner matrix dimension mismatch");
...
void invertInPlace(std::vector<std::vector<double>> &M)
{
    ...
    assert(((void)"cannot invert nonsquare matrix", nrow == ncol));
    ...
    for (size_t i = 0; i < nrow; i++)
    {
        ...
        assert(((void)"no pivot found in column: matrix is not invertible", maxentry > 0));
